...
import { Star, Clock, BookOpen, Users, PlayCircle, FileText, Award, CheckCircle, MessageCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import MainLayout from "@/components/layout/MainLayout";
import EnrollmentModal from "@/components/EnrollmentModal";
import { courses, BUSINESS_INFO } from "@/data/mockData";
...
export default function CourseDetail() {
  ...
  const whatsappEnroll = `https://wa.me/${BUSINESS_INFO.whatsapp}?text=${encodeURIComponent(`Hello HandyGidi! I'm interested in the "${course.title}" course (₦${course.price.toLocaleString()}). Please send me enrollment details.`)}`;
        ...
        <div className="container">
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1">
              ...
              <p className="opacity-80 text-sm md:text-base mb-4">{(course as any).description || "Master practical skills with hands-on projects and expert instruction."}</p>
        ...
        <div className="container flex flex-col lg:flex-row gap-8">
          <div className="flex-1">
            <Tabs defaultValue="overview">
              ...
              <TabsContent value="overview">
                ...
                <p className="text-sm text-muted-foreground mb-4">{(course as any).description}</p>
          ...
          <div className="w-full lg:w-80 shrink-0">
            <div className="bg-card border border-border rounded-lg p-6 sticky top-20">
              <img src={course.image} alt={course.title} className="w-full rounded-lg mb-4 aspect-video object-cover" />
              <p className="font-heading font-bold text-2xl text-foreground mb-1">
                {course.price === 0 ? "Sponsored / Free" : `₦${course.price.toLocaleString()}`}
              </p>
              <p className="text-xs text-muted-foreground mb-4">Flexible payment plans available</p>
              <Button className="w-full gradient-accent text-accent-foreground border-0 mb-2" size="lg" onClick={() => setEnrollOpen(true)}>
                Enroll Now
              </Button>
              <Button variant="outline" className="w-full border-green-500 text-green-600 hover:bg-green-50 mb-3" size="lg" asChild>
