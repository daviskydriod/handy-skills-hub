...
/* ─── gallery videos ─────────────────────────────────────────── */
const galleryVideos = [
  { id: 1, src: gallery1, title: "Training Session",    caption: "Live hands-on training in progress at our Abuja centre." },
  { id: 2, src: gallery2, title: "Graduation Ceremony", caption: "Celebrating our graduates as they step into their careers." },
  { id: 3, src: gallery3, title: "Community Outreach",  caption: "Taking digital skills training to communities across Abuja." },
  { id: 4, src: gallery4, title: "Workshop Highlights", caption: "Behind the scenes of our intensive weekend workshops." },
];
...
export default function Index() {
  ...
  const activeItem = galleryVideos.find(v => v.id === activeVideo) ?? null;

  const closeVideo = () => setActiveVideo(null);
  const prevVideo  = () => setActiveVideo(v => v === null ? null : v === 1 ? galleryVideos.length : v - 1);
  const nextVideo  = () => setActiveVideo(v => v === null ? null : v === galleryVideos.length ? 1 : v + 1);
    ...
    <MainLayout>
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="container max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-slate-400 text-sm mt-3 max-w-md mx-auto">
              A glimpse into our training sessions, graduations, workshops, and community activities.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {galleryVideos.map((item, i) => (
              <motion.div
                ...
                style={{ aspectRatio: "16/9", background: "#0b1f3a" }}
              ...
              >
                ...
                <div className="absolute inset-0 transition-opacity duration-300"
                  style={{ background: "linear-gradient(to top,rgba(6,13,28,0.85) 0%,rgba(6,13,28,0.2) 60%,transparent 100%)" }} />
                {/* play button */}
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110"
                    style={{ background: `linear-gradient(135deg,${GOLD},${GOLD2})`, boxShadow: `0 8px 32px rgba(234,179,8,0.45)` }}>
                    <Play size={22} className="text-white" style={{ marginLeft: 3 }} />
                  </div>
                </div>
                {/* caption */}
                <div className="absolute bottom-0 left-0 right-0 px-5 pb-5">
                  <p className="font-heading font-extrabold text-white text-sm mb-0.5">{item.title}</p>
                  <p className="text-slate-300 text-xs leading-relaxed">{item.caption}</p>
                </div>
          ...
          {/* link to full gallery on About page */}
      ...
      {activeItem && (
        <div
          ...
          style={{ background: "rgba(6,13,28,0.96)", backdropFilter: "blur(14px)" }}
          onClick={closeVideo}
        >
          <div
          ...
          >
            ...
            <div className="px-6 py-4 flex items-center justify-between gap-4">
              <div>
                <p className="font-heading font-extrabold text-white text-base">{activeItem.title}</p>
                <p className="text-slate-400 text-xs mt-0.5">{activeItem.caption}</p>
              </div>
              <span className="text-slate-500 text-xs shrink-0">{activeItem.id} / {galleryVideos.length}</span>
            </div>
            {/* close */}
            <button onClick={closeVideo}
              ...
              style={{ background: "rgba(6,13,28,0.8)", border: "1px solid rgba(255,255,255,0.15)" }}>
