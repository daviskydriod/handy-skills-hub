...
import {
  ...
  BarChart2, ChevronRight, Loader, ImageIcon, Info,
} from "lucide-react";
              ...
              {loading ? (
                ...
                <div key={c.id} className="arow">
                  ...
                  <div style={{ display:"flex", gap:4 }}>
                    {c.is_published && (
                      <button onClick={() => togglePublish(c)} className="icn" title="Unpublish"><EyeOff size={14} style={{ color:"#64748b" }} /></button>
                    )}
                    <button onClick={() => startEdit(c)} className="icn"><Edit2 size={14} style={{ color:"#3b82f6" }} /></button>
                    <button onClick={() => handleDelete(c.id, c.title)} disabled={deletingId === c.id} className="icn">
          ...
          <div style={{ maxWidth:660 }}>
            ...
            <div style={{ background:"#fffbeb", border:"1px solid #fde68a", borderRadius:12, padding:12, display:"flex", gap:8, marginBottom:22 }}>
              ...
              <p style={{ fontSize:12, color:"#92400e" }}>
                <strong>Admin approval required.</strong> Course will be saved as draft and won't appear publicly until approved.
              </p>
            </div>
        ...
        {tab === "earnings" && (
          <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
            ...
            <div style={{ background:"#fffbeb", border:"1px solid #fde68a", borderRadius:12, padding:12, display:"flex", gap:8 }}>
              ...
              <p style={{ fontSize:12, color:"#92400e" }}>Earnings are estimates based on enrolled students × price. Actual payouts depend on admin-confirmed payments.</p>
            </div>
          </div>
        )}
