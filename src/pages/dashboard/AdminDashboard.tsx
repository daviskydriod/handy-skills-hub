...
import { getCourses, deleteCourse, updateCourse, type Course } from "@/api/courses";
...
export default function AdminDashboard() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      const s: Record<number, CourseStatus> = {};
      res.courses.forEach(c => { s[c.id] = c.is_published ? "approved" : "pending"; });
      setCourseStatuses(s);
  ...
  const stats = {
    ...
    pendingCourses:   Object.values(courseStatuses).filter(s => s === "pending").length,
  ...
  const handleCourseAction = async (courseId: number, action: "approved" | "rejected") => {
    try {
      ...
      toast({ title: action === "approved" ? "Course Approved ✅" : "Course Rejected ❌" });
    ...
    <DashboardLayout items={sidebarItems} title="Admin Panel" userName={user?.name ?? "Admin"}>
      ...
      {tab === "overview" && (
        <div className="space-y-6">
          ...
          {/* Course submissions */}
          <div>
            <h2 className="font-heading font-semibold text-base mb-3 text-foreground">Course Submissions</h2>
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              {loadingCourses ? (
                <div className="p-4 space-y-3">{[1,2,3,4].map(i => <Skeleton key={i} h="h-12" />)}</div>
              ) : courses.length === 0 ? (
                      ...
                      <tr key={c.id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {c.image && <img src={c.image} alt="" className="w-8 h-8 rounded-lg object-cover shrink-0" />}
                            <span className="text-foreground font-medium text-sm line-clamp-1 max-w-[160px]">{c.title}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-muted-foreground text-xs hidden sm:table-cell">{c.instructor}</td>
                        <td className="px-4 py-3"><StatusBadge status={courseStatuses[c.id] ?? "pending"} /></td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1.5">
                            {courseStatuses[c.id] !== "approved" && (
                              <Button size="sm" onClick={() => handleCourseAction(c.id, "approved")}
                                className="h-7 text-xs bg-emerald-500 hover:bg-emerald-600 text-white border-0 px-3">
                                Approve
                              </Button>
                            )}
                            {courseStatuses[c.id] !== "rejected" && (
                              <Button size="sm" onClick={() => handleCourseAction(c.id, "rejected")}
                                className="h-7 text-xs bg-red-500 hover:bg-red-600 text-white border-0 px-3">
                                Reject
                              </Button>
                            )}
                          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-5">
            ...
            <div>
              <h2 className="font-heading font-semibold text-base mb-3 text-foreground">Top Courses by Enrollment</h2>
              <div className="bg-card border border-border rounded-xl divide-y divide-border">
                {loadingCourses
                  ? [1,2,3,4,5].map(i => <div key={i} className="p-3"><Skeleton h="h-9" /></div>)
                  ...
                  <tbody>
                    {filteredUsers.map(u => (
                      <tr key={u.id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-semibold border ${u.is_active ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-red-50 text-red-600 border-red-200"}`}>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-secondary/40 border-b border-border">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs hidden md:table-cell">Enrolled</th>
                      <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Status</th>
                      <th className="px-4 py-3 text-right font-semibold text-muted-foreground text-xs">Actions</th>
                    </tr>
                  </thead>
