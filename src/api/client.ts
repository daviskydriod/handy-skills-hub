...
client.interceptors.request.use((config) => {
  const token = localStorage.getItem("hg_token"); // ✅ FIXED: was "token"
...
client.interceptors.response.use(
  ...
  (err) => {
    ...
    if (status === 401 && isAuthMe) {
      localStorage.removeItem("hg_token"); // ✅ FIXED: was "token"
      localStorage.removeItem("hg_user");  // ✅ FIXED: also clear user
